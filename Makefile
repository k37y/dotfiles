...
NVIM_LINUX_AMD64_LATEST=$(shell curl -s https://api.github.com/repos/neovim/neovim/releases/latest | jq -r '.assets | .[] | select(.name | contains("linux-x86_64.tar")) | .browser_download_url' | head -1)
NVIM_DARWIN_ARM64_LATEST=$(shell curl -s https://api.github.com/repos/neovim/neovim/releases/latest | jq -r '.assets | .[] | select(.name | contains("macos-arm64.tar")) | .browser_download_url' | head -1)
